...
const useDownloadLogic = (id: string) => {
  ...
  return { downloadOffset, hiddenCanvasRef, isDownloading, setDownloadOffset, setIsDownloading };
};

// --- Sub Components ---

export const SaveActions: React.FC<CommonProps> = ({ id }) => {
  const { downloadOffset, hiddenCanvasRef, isDownloading, setDownloadOffset, setIsDownloading } = useDownloadLogic(id);
  const futureUrl = downloadOffset !== null ? generateUrl(id, downloadOffset) : '';
    ...
    <>
      <div className="w-full my-5 bg-white p-3 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-3 gap-1.5 sm:grid-cols-5">
           <DownloadBtn label="Now" offset={0} onClick={handleDownload} loading={isDownloading} />
           <DownloadBtn label="+5s" offset={5} onClick={handleDownload} loading={isDownloading} />
           <DownloadBtn label="+10s" offset={10} onClick={handleDownload} loading={isDownloading} />
           <DownloadBtn label="+30s" offset={30} onClick={handleDownload} loading={isDownloading} />
           <DownloadBtn label="+60s" offset={60} onClick={handleDownload} loading={isDownloading} />
        </div>
      </div>
      {/* Hidden Render Area */}
...
export const QRCodeMinimal: React.FC<CommonProps & {  size: number }> = ({ id,  size }) => {
...
const DownloadBtn: React.FC<{ label: string, offset: number, onClick: (o: number) => void, loading: boolean }> = ({ label, offset, onClick, loading }) => (
    <button 
        disabled={loading}
